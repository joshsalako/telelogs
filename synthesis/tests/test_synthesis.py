...
3. **Evaluating C3 (Neighbor Cell Higher Throughput):** Look for a handover event (the `5G KPI PCell RF Serving PCI` changes). Check the `5G KPI PCell Layer2 MAC DL Throughput [Mbps]`. If the throughput is low (e.g., < 50 Mbps) BEFORE the handover, but immediately recovers to a high value (e.g., > 300 Mbps) AFTER the handover, the neighbor provides higher throughput.
4. **Evaluating C4 (Non-colocated Overlapping Coverage):** Identify the `5G KPI PCell RF Serving PCI` and the `Measurement PCell Neighbor Cell Top Set(Cell Level) Top 1 PCI`. Look up BOTH of these exact PCIs in the "PCI" column of the Engineering parameters table. If they both have the EXACT SAME `gNodeB ID`, they are colocated on the same tower. You MUST explicitly rule out C4.
5. **Evaluating C5 (Frequent Handovers):** Scan the `5G KPI PCell RF Serving PCI` column from top to bottom. Count every time the value changes from one row to the next. If it changes 3 or more times back-and-forth, it is frequent. If it changes 0, 1, or 2 times, it is NOT frequent, and you MUST explicitly rule out C5.
